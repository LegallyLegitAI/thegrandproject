...
import { pricingTiers } from './components/data';
import { Icon } from './components/Icon';

export default function PricingPage() {
  const { state, dispatch } = useStateContext();
  const { pricingPeriod, userTier, emailGateCompleted, user } = state;
  ...
  const handleSubscribe = (priceId: string) => {
    if (!emailGateCompleted) {
      ...
      dispatch({
        type: 'ADD_TOAST',
        payload: { id: Date.now(), message: 'Please complete the free health check first!', type: 'info' },
      });
      return;
    }
    // Use user's email from state for checkout
    initiateCheckout(dispatch, priceId, 'subscription', user?.email || '');
  };
      ...
      <div className="pricing-toggle-container">
        ...
        <label className="toggle-switch" aria-label="Toggle between monthly and annual pricing">
          <input
            ...
            onChange={() =>
              dispatch({
                type: 'SET_PRICING_PERIOD',
                payload: pricingPeriod === 'monthly' ? 'annual' : 'monthly',
              })
            }
          />
          ...
          return (
            <div key={key} className={`pricing-card ${isPopular ? 'popular' : ''}`}>
              ...
              <p className="price-desc">
                {key === 'essentials' && pricingPeriod === 'annual'
                  ? 'Save $98 compared to monthly!'
                  : key === 'pro' && pricingPeriod === 'annual'
                  ? 'Save $198 compared to monthly!'
                  : ' '}
              </p>
