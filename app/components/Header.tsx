...
export default function Header() {
    ...
    const isLoggedIn = state.emailGateCompleted;
    ...
    return (
        <header className={`app-header ${state.isNavOpen ? 'nav-open' : ''}`}>
            ...
            <nav className={`app-nav ${state.isNavOpen ? 'mobile-active' : ''}`}>
                {isLoggedIn && (
                    <>
            ...
            <div className="header-actions">
                {isLoggedIn ? (
                    <div className="user-status">
                        <span className="plan-badge">{state.userTier}</span>
                        <a href="#" onClick={handleLogout} className="subtle-text" style={{ textDecoration: 'none' }}>Logout</a>
                    </div>
                ) : (
                    <button className="btn btn-primary" onClick={() => navigateTo(dispatch, 'healthCheck')}>Get Free Score</button>
                )}
